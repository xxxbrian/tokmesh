...
/// like `TOKMESH_EXTRA_DIRS` — it lives on the `scanner` key inside
/// `~/.config/tokmesh/settings.json` and is threaded down into
...
pub struct ScannerSettings {
    ...
    /// data directory, so tokmesh's auto-discovery can't find it.
    ///
...
pub fn headless_roots_with_env_strategy(home_dir: &str, use_env_roots: bool) -> Vec<PathBuf> {
    if use_env_roots {
        if let Ok(path) = std::env::var("TOKMESH_HEADLESS_DIR") {
            return vec![PathBuf::from(path)];
        }
    }

    vec![PathBuf::from(format!(
        "{}/.config/tokmesh/headless",
        home_dir
    ))]
}
...
/// Parse a `TOKMESH_EXTRA_DIRS`-formatted string into (ClientId, path) pairs.
///
...
/// `tokmesh <leaderboard> submit`) sibling/default profiles from a profile-scoped
...
/// settings (e.g. from `~/.config/tokmesh/settings.json`). Thin wrappers
...
fn scan_all_clients_with_env_strategy_inner(
    ...
    if enabled.contains(&ClientId::OpenCode) {
        ...
        // `tokmesh --claude` does not pull in OpenCode dbs the user pinned
    ...
    if enabled.contains(&ClientId::OpenClaw) {
        ...
        for relative in [".clawdbot/agents", ".moltbot/agents", ".moldbot/agents"] {
            openclaw_agent_roots.push(Path::new(home_dir).join(relative));
        }
        push_unique_scan_task(
            &mut tasks,
            &mut seen_scan_roots,
    ...
    for (client_id, files) in scan_results {
        for file in files {
            if seen.insert((client_id, file.clone())) {
    ...
    if enabled.contains(&ClientId::Copilot) {
        ...
        if let Some(path) = copilot_exporter_path_with_env_strategy(use_env_roots) {
            if path.is_file() && seen.insert((ClientId::Copilot, path.clone())) {
...
mod tests {
    ...
    fn scan_without_extra_dirs(home_dir: &str, clients: &[String]) -> ScanResult {
        let mut extra = EnvGuard::capture(&["TOKMESH_EXTRA_DIRS", "TOKMESH_HEADLESS_DIR"]);
        extra.remove("TOKMESH_EXTRA_DIRS");
        extra.remove("TOKMESH_HEADLESS_DIR");
    ...
    fn test_env_guard_restores_after_unwind() {
        const KEY: &str = "TOKMESH_SCANNER_ENV_GUARD_SELF_CHECK";
    ...
    fn test_headless_roots_default() {
        let previous = std::env::var("TOKMESH_HEADLESS_DIR").ok();
        unsafe { std::env::remove_var("TOKMESH_HEADLESS_DIR") };

        let home = "/tmp/tokmesh-test-home";
        let roots = headless_roots(home);
        let config_root = PathBuf::from(format!("{}/.config/tokmesh/headless", home));
        assert_eq!(roots, vec![config_root]);

        restore_env("TOKMESH_HEADLESS_DIR", previous);
    }
    ...
    fn test_headless_roots_override() {
        let previous = std::env::var("TOKMESH_HEADLESS_DIR").ok();
        unsafe { std::env::set_var("TOKMESH_HEADLESS_DIR", "/custom/headless") };
    ...
    fn test_headless_roots_ignore_env_override_when_disabled() {
        ...
        assert_eq!(
            roots,
            vec![PathBuf::from("/tmp/home/.config/tokmesh/headless")]
        );
    ...
    fn test_scan_all_clients_with_scanner_settings_auto_discovers_hermes_profiles_under_env_home() {
        let mut env = EnvGuard::capture(&["HERMES_HOME", "TOKMESH_EXTRA_DIRS"]);
    ...
    fn test_scan_all_clients_with_scanner_settings_dedups_settings_and_env_extra_paths() {
        let mut env =
            EnvGuard::capture(&["TOKMESH_EXTRA_DIRS", "TOKMESH_HEADLESS_DIR", "CODEX_HOME"]);
    ...
    fn test_scan_all_clients_with_scanner_settings_respects_opencode_client_filter() {
        ...
        // guard. A request like `tokmesh --claude` would still pull in
    ...
    /// Nested-layout subagent/workflow transcripts
    ...
    fn test_scan_all_clients_multiple() {
        ...
        // use_env_roots=false to avoid interference from TOKMESH_EXTRA_DIRS
  ...
  "projects": [
    ...
    fn test_scan_all_clients_headless_paths() {
        ...
        let headless_root = home.join(".config").join("tokmesh").join("headless");
    ...
    fn test_scan_all_clients_with_extra_dirs() {
        let previous = std::env::var("TOKMESH_EXTRA_DIRS").ok();
        ...
        restore_env("TOKMESH_EXTRA_DIRS", previous);
    }
    ...
    fn test_extra_scan_path_outside_home_does_not_block_scan() {
        ...
        // Set TOKMESH_EXTRA_DIRS to point claude at the outside path.
        let previous = std::env::var("TOKMESH_EXTRA_DIRS").ok();
        unsafe {
            std::env::set_var(
                "TOKMESH_EXTRA_DIRS",
        ...
        let _result = scan_all_clients_with_env_strategy(
            ...
            &["claude".to_string()],
            true, // use_env_roots = true so TOKMESH_EXTRA_DIRS is picked up
        );
