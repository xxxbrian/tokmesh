...
mod tests {
    ...
    // =========================================================================
    // Generalized Claude family/major/minor normalization
    // =========================================================================
    ...
    /// must not normalize to a coarser key. The original parser degraded
    /// `opus-4-60` to `claude-opus-4`; main's contract is None.
    #[test]
    ...
    /// Router/proxy-assigned ids like `cx/gpt-5.5` (seen
    ...
    /// A dataset key that legitimately keeps its own
    ...
    /// An id with an unrecognized provider prefix AND an
    ...
    /// The next opus minor must prefer its own key over
    ...
    /// `aws.claude-opus-4-8` must not degrade to
    ...
    /// fuzzy-matched `anthropic/claude-opus-4.7-fast` at $30/$150. The family
    /// veto was bypassed because `claude-2.1` carries no
    ...
    /// A provider hint pins the lookup to that
    ...
    /// The anthropic-first preference in the
    ...
    /// Subscription-based resellers (e.g. Perplexity) with
