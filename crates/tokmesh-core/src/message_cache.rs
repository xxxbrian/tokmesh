...
fn user_scoped_temp_dir() -> Option<PathBuf> {
    ...
    Some(std::env::temp_dir().join(format!("tokmesh-uid-{uid}")))
}
...
fn user_scoped_temp_dir() -> Option<PathBuf> {
    std::env::var_os("USERNAME")
        ...
        .map(|user| {
            ...
            path.push(format!("tokmesh-user-{}", user.to_string_lossy()));
            path
        })
}
...
fn warn_cache_failure_once(context: &'static str, path: &Path, error: &impl std::fmt::Display) {
    ...
    {
        eprintln!("tokmesh: warning: {context} ({}): {error}", path.display());
    }
}
...
fn parser_version(client: ClientId) -> u32 {
    match client {
        ...
        // `completed_at`. v2->v3: rows with a NULL
        ...
        // This completes the start-anchor correction for zcode.
        ClientId::Zcode => 3,
        ...
        // to the start-anchor correction.
        ClientId::Kiro => 2,
...
mod tests {
    ...
    fn sandbox_cache_env(
        ...
        let prev_override = std::env::var_os("TOKMESH_CONFIG_DIR");
        unsafe {
    ...
    fn restore_cache_env(
        ...
        restore_env_var("TOKMESH_CONFIG_DIR", prev.3);
    }
    ...
    fn test_duration_anchor_audit_remaining_parsers_bumps_versions() {
        // Junie, jcode, devin-cli, zcode, and
...
"#,
    ...
    fn test_fallback_cache_dir_prefers_runtime_dir() {
...

        {
            assert_eq!(
                fallback_cache_dir(),
                Some(runtime_dir.path().join("tokmesh"))
            );
        }
