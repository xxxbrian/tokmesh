...
struct Cli {
    ...
    #[arg(
        ...
        help = "After --light renders, atomically overwrite the TUI cache with this report's data so the next `tokmesh tui` starts from fresh data. Persists across invocations via settings.json `light.writeCache`."
    )]
    write_cache: bool,
...
enum Commands {
    ...
    #[command(
        about = "Import historical usage from a third-party aggregate export (e.g. clawdboard) into tokmesh JSON"
    )]
    Import {
        ...
        #[arg(
            long,
            help = "Write normalized tokmesh JSON to this file instead of stdout"
        )]
        output: Option<String>,
    ...
    Wrapped {
        #[arg(long, help = "Output file path (default: tokmesh-{year}-wrapped.png)")]
        output: Option<String>,
...
pub fn run() -> Result<()> {
    ...
    tokmesh_core::model_alias::set_global(&tui::settings::load_model_aliases_for_home(&cli.home));
    let opencode_model_names = tokmesh_core::opencode_model_name::load_for_home(
        cli.home.as_deref().map(std::path::Path::new),
    );
    tokmesh_core::opencode_model_name::set_global(opencode_model_names);
    ...
    match cli.command {
        ...
        None => {
            ...
            let group_by: tokmesh_core::GroupBy = cli.group_by.parse().unwrap_or_else(|e| {
                eprintln!("Error: {}", e);
                std::process::exit(1);
            });
...
impl ClientFilter {
    ...
    pub fn to_client_id(self) -> Option<tokmesh_core::ClientId> {
    ...
    pub fn from_client_id(client: tokmesh_core::ClientId) -> Self {
        use tokmesh_core::ClientId;
        match client {
...
/// Builds the client filter list passed to `tokmesh_core`.
///
/// Resolution order:
/// 1. Collect canonical `--client/-c` values (preserves user order).
/// 2. If step 1 produced nothing, fall back to user-configured
///    `defaultClients` from `~/.config/tokmesh/settings.json` when present.
...
fn cursor_setup_state(home_dir: &Option<String>) -> Option<CursorSetupState> {
    ...
    let cache_glob = if home_override {
        home_path
            .join(".config/tokmesh/cursor-cache/usage*.csv")
            .to_string_lossy()
            .to_string()
    } else {
        "~/.config/tokmesh/cursor-cache/usage*.csv".to_string()
    };
...
fn cursor_setup_warnings_for_report(
    ...
    let Some(state) = cursor_setup_state(home_dir) else {
        return vec![
            "Cursor usage requires Tokmesh's Cursor API cache, but the home directory could not be resolved. Run `tokmesh cursor login` (auto-detects Cursor desktop when signed in) and `tokmesh cursor sync --json`. Tokmesh does not parse local `~/.cursor` session data.".to_string(),
        ];
    };
    if state.has_cache {
        return Vec::new();
    ...
    let action = if state.home_override {
        "run `tokmesh cursor login` (auto-detects Cursor desktop when signed in) and `tokmesh cursor sync --json`, or populate that cache before running a report with --home"
    } else if state.has_credentials {
        "run `tokmesh cursor sync --json`"
    } else {
        "run `tokmesh cursor login` (auto-detects Cursor desktop when signed in) and `tokmesh cursor sync --json`"
    };

    vec![format!(
        "Cursor usage requires Tokmesh's Cursor API cache at `{}`; {}. Tokmesh does not parse local `~/.cursor` session data.",
...
fn warp_setup_warnings_for_report(
    ...
    let (home_path, home_override) = match home_dir {
        ...
        None => match dirs::home_dir() {
            ...
            None => {
                return vec![
                    "Warp usage requires Tokmesh's Warp aggregate cache, but the home directory could not be resolved. Tokmesh does not parse local Warp transcripts.".to_string(),
                ];
            }
        },
    };
    ...
    let cache_glob = if home_override {
        ...
        "~/.config/tokmesh/warp-cache/usage*.json".to_string()
    };
    let action = if home_override {
        "run `tokmesh warp sync` for the default profile or populate that cache before running a report with --home"
    } else if warp::has_credentials() {
        "run `tokmesh warp sync`"
    } else {
        "run `tokmesh warp login` and `tokmesh warp sync`"
    };

    vec![format!(
        "Warp usage requires Tokmesh's aggregate API cache at `{}`; {}. Tokmesh does not parse local Warp/Oz session transcripts and does not infer tokens from request counts.",
        cache_glob, action
    )]
}
...
fn auto_sync_cursor_for_local_report(
    ...
    // recent enough — running `tokmesh models` 30× in a script must not
    // produce 30 Cursor API calls. The manual `tokmesh cursor sync` command
    // bypasses this gate.
...
fn model_usage_includes_client(entry: &tokmesh_core::ModelUsage, client: &str) -> bool {
...
fn run_models_report(
    ...
    use tokmesh_core::{get_model_report, GroupBy, ReportOptions};
    ...
    if json {
        ...
        struct ModelUsageJson {
            ...
            message_count: i32,
            cost: f64,
            performance: tokmesh_core::ModelPerformance,
        }
...
fn run_monthly_report(
    ...
    use std::time::Instant;
    use tokio::runtime::Runtime;
    use tokmesh_core::{get_monthly_report, GroupBy, ReportOptions};
...
fn run_hourly_report(
    ...
    use std::time::Instant;
    use tokio::runtime::Runtime;
    use tokmesh_core::{get_hourly_report, GroupBy, ReportOptions};
...
fn run_pricing_list_overrides(json: bool) -> Result<()> {
    use colored::Colorize;
    use tokmesh_core::pricing::custom::CustomPricing;
    use tokmesh_core::pricing::ModelPricing;
    ...
    println!("  Loaded once at startup; restart tokmesh after editing this file.");
    println!();
...
/// grand-total token counts. tokmesh-core saturates these fields at the
...
fn monthly_token_field_totals(entries: &[tokmesh_core::MonthlyUsage]) -> (i64, i64, i64, i64) {
    entries.iter().fold(
        (0, 0, 0, 0),
...
fn model_entry_total_tokens(entry: &tokmesh_core::ModelUsage) -> i64 {
    // saturating_add (mirrors tokmesh_core::TokenBreakdown::total) so a
...
fn aggregate_model_report_performance(
    entries: &[tokmesh_core::ModelUsage],
) -> tokmesh_core::ModelPerformance {
    let mut performance = tokmesh_core::ModelPerformance::default();
    for entry in entries {
...
fn run_clients_command(json: bool, home_dir: Option<String>) -> Result<()> {
    ...
    let headless_roots =
        tokmesh_core::scanner::headless_roots_with_env_strategy(&home_dir_str, use_env_roots);
    ...
    let extra_dirs: Vec<(ClientId, String)> = if use_env_roots {
        let extra_dirs_val = std::env::var("TOKMESH_EXTRA_DIRS").unwrap_or_default();
        tokmesh_core::parse_extra_dirs(&extra_dirs_val, &all_clients)
    } else {
        Vec::new()
    };
    ...
    let copilot_exporter_path =
        tokmesh_core::copilot_exporter_path_with_env_strategy(use_env_roots);
        ...
        ClientId::iter()
            .map(|client| {
                ...
                if client == ClientId::DevinDesktop {
                    for root in tokmesh_core::scanner::devin_desktop_additional_roots(
                        &home_dir_str,
                        use_env_roots,
                    ) {
...
fn get_headless_roots(home_dir: &Path) -> Vec<PathBuf> {
    ...
    if let Ok(env_dir) = std::env::var("TOKMESH_HEADLESS_DIR") {
        roots.push(PathBuf::from(env_dir));
    } else {
        roots.push(home_dir.join(".config/tokmesh/headless"));
    }
...
fn run_time_metrics_report(
    ...
    use tokio::runtime::Runtime;
    use tokmesh_core::{get_time_metrics_report, GroupBy, ReportOptions};
    ...
    if json {
        ...
        struct TimeMetricsReportJson<'a> {
            metrics: &'a tokmesh_core::TimeMetrics,
            processing_time_ms: u32,
            #[serde(skip_serializing_if = "Vec::is_empty")]
            warnings: Vec<String>,
        }
...
fn run_graph_command(
    ...
    use colored::Colorize;
    use std::time::Instant;
    use tokmesh_core::{generate_local_graph_report, GroupBy, ReportOptions};
...
/// standard tokmesh JSON — the same shape `tokmesh graph` produces.
///
...
fn run_import_command(
    ...
    // stays pure JSON when no --output path is given (matching `tokmesh
    // graph`'s behavior) — e.g. `tokmesh import export.json > out.json`
...
///
/// 1. If `defaultClients` from `~/.config/tokmesh/settings.json` is
...
fn write_light_cache(
    ...
    if home_dir.is_some() {
        eprintln!(
            "tokmesh: --write-cache skipped because --home is set; \
    ...
    let scan_clients: Vec<tokmesh_core::ClientId> = enabled_set
        .iter()
...
fn run_headless_command(
    ...
    let output_path = if let Some(custom_output) = output {
        ...
        let root = headless_roots
            .first()
            .cloned()
            .unwrap_or_else(|| home_dir.join(".config/tokmesh/headless"));
    ...
    if outcome.timed_out {
        ...
        eprintln!("{}", "  Partial output saved. Increase timeout with TOKMESH_NATIVE_TIMEOUT_MS or settings.json".bright_black());
        println!();
...
mod tests {
    ...
    fn saturating_token_total_saturates_instead_of_overflowing() {
        // tokmesh-core clamps corrupt per-field token buckets to
    ...
    fn monthly_token_field_totals_saturate_across_entries() {
        ...
        let make = |input: i64| tokmesh_core::MonthlyUsage {
    ...
    fn model_entry_total_tokens_saturates_a_single_entrys_buckets() {
        let entry = tokmesh_core::ModelUsage {
            ...
            message_count: 1,
            cost: 0.0,
            performance: tokmesh_core::ModelPerformance::default(),
        };
    ...
    fn aggregate_model_report_performance_saturates_cross_entry_total() {
        ...
        let make = || tokmesh_core::ModelUsage {
            client: "antigravity-cli".to_string(),
            merged_clients: None,
            workspace_key: None,
            workspace_label: None,
    ...
    // reads from `~/.config/tokmesh/settings.json`. Reading host config
    ...
    fn test_removed_legacy_client_flags_now_error() {
        for flag in REMOVED_LEGACY_CLIENT_FLAGS {
            let arg = format!("--{flag}");
            let result = Cli::try_parse_from(["tokmesh", arg.as_str()]);
            assert!(
                result.is_err(),
    ...
    fn test_canonical_client_still_parses_for_removed_flag_names() {
        ...
        for flag in REMOVED_LEGACY_CLIENT_FLAGS {
            let cli = Cli::try_parse_from(["tokmesh", "--client", flag])
    ...
    fn test_canonical_client_parses_single_and_multi() {
        let cli = Cli::try_parse_from(["tokmesh", "--client", "opencode"]).expect("parse ok");
        assert_eq!(
            build_client_filter_with_defaults(cli.clients, &[]),
        ...
        let cli =
            Cli::try_parse_from(["tokmesh", "--client", "opencode,claude"]).expect("parse ok");
        assert_eq!(
            build_client_filter_with_defaults(cli.clients, &[]),
            Some(vec!["opencode".to_string(), "claude".to_string()])
        );

        let cli = Cli::try_parse_from(["tokmesh", "--client", "synthetic"]).expect("parse ok");
        assert_eq!(
            build_client_filter_with_defaults(cli.clients, &[]),
    ...
    fn test_client_filter_as_filter_str_matches_client_id_for_overlap() {
        ...
        for filter in ClientFilter::value_variants() {
            ...
            assert!(
                tokmesh_core::ClientId::from_str(id).is_some(),
    ...
    fn test_client_filter_order_matches_client_id_all() {
        ...
        let filters: Vec<ClientFilter> = ClientFilter::value_variants()
            ...
            .collect();
        let ids: Vec<tokmesh_core::ClientId> = tokmesh_core::ClientId::ALL.to_vec();
    ...
    fn test_build_client_filter_defaults_dropped_for_unknown_ids() {
        ...
        // dropped, never errors. Ensures a typo never breaks tokmesh.
        let flags = ClientFlags::default();
    ...
    fn test_client_flags_parses_canonical_form() {
        ...
        let cli =
            Cli::try_parse_from(["tokmesh", "-c", "opencode", "-c", "claude"]).expect("parse ok");
        assert_eq!(
            cli.clients.clients,
            vec![ClientFilter::Opencode, ClientFilter::Claude]
        );
    ...
    fn test_wrapped_parses_clients_view_flag() {
        let cli = Cli::try_parse_from(["tokmesh", "wrapped"]).expect("parse ok");
        ...
        let cli = Cli::try_parse_from(["tokmesh", "wrapped", "--clients"]).expect("parse ok");
    ...
    fn test_wrapped_client_filter_coexists_with_clients_view_flag() {
        let cli =
            Cli::try_parse_from(["tokmesh", "wrapped", "--client", "opencode"]).expect("parse ok");
        let Some(Commands::Wrapped {
            client_flags,
            show_clients,
            ..
        ...
        let cli = Cli::try_parse_from(["tokmesh", "wrapped", "--clients", "--client", "opencode"])
            .expect("parse ok");
        let Some(Commands::Wrapped {
            client_flags,
            show_clients,
    ...
    fn test_client_flag_accepts_uppercase() {
        let cli =
            Cli::try_parse_from(["tokmesh", "--client", "OPENCODE"]).expect("uppercase parses");
        assert_eq!(cli.clients.clients, vec![ClientFilter::Opencode]);

        let cli = Cli::try_parse_from(["tokmesh", "-c", "Codebuff,Antigravity"])
    ...
    fn test_client_flag_rejects_unknown_and_empty_values() {
        assert!(Cli::try_parse_from(["tokmesh", "--client", "unknown"]).is_err());
        assert!(Cli::try_parse_from(["tokmesh", "--client", ""]).is_err());
    }
    ...
    fn test_client_filter_goose_round_trip() {
        ...
        assert_eq!(
            ...
            Some(tokmesh_core::ClientId::Goose)
        );
        assert_eq!(
            ClientFilter::from_client_id(tokmesh_core::ClientId::Goose),
            ClientFilter::Goose
        );
    }
    ...
    fn test_client_filter_zed_round_trip() {
        ...
        assert_eq!(
            ...
            Some(tokmesh_core::ClientId::Zed)
        );
        assert_eq!(
            ClientFilter::from_client_id(tokmesh_core::ClientId::Zed),
            ClientFilter::Zed
        );
    }
    ...
    fn test_date_shortcut_flags_conflict() {
        let err = expect_parse_error(&["tokmesh", "--today", "--yesterday"]);
        assert_eq!(err.kind(), clap::error::ErrorKind::ArgumentConflict);

        let err = expect_parse_error(&["tokmesh", "--week", "--month"]);
        assert_eq!(err.kind(), clap::error::ErrorKind::ArgumentConflict);
    }

    #[test]
    fn test_date_shortcut_conflicts_with_since_until_year() {
        let err = expect_parse_error(&["tokmesh", "--today", "--since", "2024-01-01"]);
        assert_eq!(err.kind(), clap::error::ErrorKind::ArgumentConflict);

        let err = expect_parse_error(&["tokmesh", "--week", "--until", "2024-12-31"]);
        assert_eq!(err.kind(), clap::error::ErrorKind::ArgumentConflict);

        let err = expect_parse_error(&["tokmesh", "--month", "--year", "2024"]);
        assert_eq!(err.kind(), clap::error::ErrorKind::ArgumentConflict);
    }

    #[test]
    fn test_date_shortcut_conflict_applies_to_subcommands() {
        let err = expect_parse_error(&["tokmesh", "models", "--today", "--yesterday"]);
        assert_eq!(err.kind(), clap::error::ErrorKind::ArgumentConflict);
    }
    ...
    fn clap_rejects_write_cache_without_light() {
        assert!(Cli::try_parse_from(["tokmesh", "--write-cache"]).is_err());
    }

    #[test]
    fn clap_rejects_no_write_cache_without_light() {
        assert!(Cli::try_parse_from(["tokmesh", "--no-write-cache"]).is_err());
    }

    #[test]
    fn clap_rejects_both_write_flags_together() {
        assert!(
            Cli::try_parse_from(["tokmesh", "--light", "--write-cache", "--no-write-cache",])
                .is_err()
        );
    }
    ...
    fn clap_accepts_models_light_write_cache_after_subcommand() {
        assert!(Cli::try_parse_from(["tokmesh", "models", "--light", "--write-cache"]).is_ok());
    }

    #[test]
    fn clap_accepts_cursor_sync_command() {
        assert!(Cli::try_parse_from(["tokmesh", "cursor", "sync"]).is_ok());
        assert!(Cli::try_parse_from(["tokmesh", "cursor", "sync", "--json"]).is_ok());
    }

    #[test]
    fn clap_accepts_codex_account_commands() {
        assert!(Cli::try_parse_from(["tokmesh", "codex", "import", "--name", "work"]).is_ok());
        assert!(Cli::try_parse_from(["tokmesh", "codex", "accounts"]).is_ok());
        assert!(Cli::try_parse_from(["tokmesh", "codex", "accounts", "--json"]).is_ok());
        assert!(Cli::try_parse_from(["tokmesh", "codex", "switch", "work"]).is_ok());
        assert!(Cli::try_parse_from(["tokmesh", "codex", "remove", "work"]).is_ok());
        assert!(Cli::try_parse_from(["tokmesh", "codex", "status"]).is_ok());
        assert!(Cli::try_parse_from(["tokmesh", "codex", "status", "--name", "work"]).is_ok());
        assert!(
            Cli::try_parse_from(["tokmesh", "codex", "status", "--name", "work", "--json"]).is_ok()
        );
    }

    #[test]
    fn clap_accepts_warp_status_and_sync_commands() {
        assert!(Cli::try_parse_from(["tokmesh", "warp", "status"]).is_ok());
        assert!(Cli::try_parse_from(["tokmesh", "warp", "status", "--json"]).is_ok());
        assert!(Cli::try_parse_from(["tokmesh", "warp", "sync"]).is_ok());
        assert!(Cli::try_parse_from(["tokmesh", "warp", "sync", "--json"]).is_ok());
    }
    ...
    fn client_filter_round_trips_warp() {
        ...
        assert_eq!(
            ...
            Some(tokmesh_core::ClientId::Warp)
        );
        assert_eq!(
            ClientFilter::from_client_id(tokmesh_core::ClientId::Warp),
            ClientFilter::Warp
        );
    }
    ...
    fn client_filter_round_trips_grok() {
        ...
        assert_eq!(
            ...
            Some(tokmesh_core::ClientId::Grok)
        );
        assert_eq!(
    ...
    fn warp_setup_warning_explains_missing_aggregate_cache() {
        ...
        assert!(warnings[0].contains("tokmesh warp"));
    ...
    fn write_light_cache_refuses_when_home_dir_set() {
        ...
        let group_by = tokmesh_core::GroupBy::default();
        write_light_cache(
