...
/// `~/.config/tokmesh/settings.json` so data-loader unit tests stay
/// hermetic across developer machines; production builds still honor
/// user-configured paths.
#[cfg(not(test))]
fn data_loader_scanner_settings() -> tokmesh_core::scanner::ScannerSettings {
    crate::tui::settings::load_scanner_settings()
}

#[cfg(test)]
fn data_loader_scanner_settings() -> tokmesh_core::scanner::ScannerSettings {
    tokmesh_core::scanner::ScannerSettings::default()
}
    ...
    fn load_with_pricing(
        ...
        let messages = if Handle::try_current().is_ok() {
            std::thread::scope(|s| {
                s.spawn(|| {
                    let rt = Runtime::new().map_err(|e| e.to_string())?;
                    rt.block_on(tokmesh_core::parse_local_unified_messages_with_pricing(
                        opts,
                        Some(pricing),
                    ))
                })
            ...
            Runtime::new()?.block_on(tokmesh_core::parse_local_unified_messages_with_pricing(
                opts,
                Some(pricing),
            ))
        }
...
/// CLI hourly bucketing behavior in `tokmesh-core::lib::get_hourly_report`.
...
mod tests {
    ...
    use tokio::runtime::{Handle, Runtime};
    use tokmesh_core::parse_local_unified_messages_with_pricing;
    use tokmesh_core::pricing::{ModelPricing, PricingService};
    use tokmesh_core::TokenBreakdown as CoreTokenBreakdown;

    #[test]
    fn positive_unified_token_total_saturates_instead_of_overflowing() {
        // tokmesh-core clamps corrupt per-field token buckets to i64::MAX; a
    ...
    fn test_data_loader_scanner_settings_is_hermetic_under_cfg_test() {
        ...
        // `~/.config/tokmesh/settings.json`. Otherwise every DataLoader
...
after"#,
    ...
    fn test_session_models_dedup_by_display_name_and_retain_provider() {
        ...
        let tokens = || tokmesh_core::TokenBreakdown {
            input: 10,
            output: 5,
            cache_read: 0,
            cache_write: 0,
