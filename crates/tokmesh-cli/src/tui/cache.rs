...
use tokmesh_core::{sessions, GroupBy, ModelPerformance};
...
/// fired after `tokmesh <leaderboard> submit` — must use this exact value, NOT
/// `GroupBy::default()`.
///
...
pub fn load_cache(
    ...
    let cached: Option<CachedTUIData> = File::open(&cache_path)
        .ok()
...
pub fn save_cached_data(
    ...
    if serde_json::to_writer(writer, &cached).is_ok() {
        let _ = tokmesh_core::fs_atomic::replace_file(&temp_path, &cache_path);
    } else {
...
mod tests {
  ...
  "data": {
    ...
    fn save_cached_data_writes_report_scope() {
        let temp_dir = TempDir::new().unwrap();
        let previous_home = env::var_os("HOME");
        let previous_override = env::var_os("TOKMESH_CONFIG_DIR");
        unsafe {
            env::set_var("HOME", temp_dir.path());
        ...
        match previous_override {
            Some(value) => unsafe { env::set_var("TOKMESH_CONFIG_DIR", value) },
            None => unsafe { env::remove_var("TOKMESH_CONFIG_DIR") },
        }
    }
  ...
  "data": {{
    ...
    ///
    /// Symptom: `tokmesh` (TUI launch) silently dropped the
    /// on-disk cache and showed an empty dashboard until the background
    /// scan finished, even though `~/.config/tokmesh/cache/tui-data-cache.json`
    ...
    /// `tokmesh <leaderboard> submit`) saved the cache with `GroupBy::default()`
