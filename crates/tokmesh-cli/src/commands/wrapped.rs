...
async fn generate_wrapped(options: WrappedOptions) -> Result<String> {
    ...
    let output = options
        .output
        .clone()
        .unwrap_or_else(|| format!("tokmesh-{}-wrapped.png", data.year));
...
fn build_top_agents(parsed: &tokmesh_core::ParsedMessages) -> Vec<WrappedAgentEntry> {
    ...
    for message in &parsed.messages {
        ...
        let normalized = tokmesh_core::sessions::normalize_opencode_agent_name(agent);
...
fn cursor_setup_warning_for_wrapped(
    ...
    Some(format!(
        "Cursor usage requires Tokmesh's Cursor API cache at `~/.config/tokmesh/cursor-cache/usage*.csv`; {action}. Tokmesh does not parse local `~/.cursor` session data."
    ))
}
...
mod cursor_setup_warning_tests {
    ...
    fn wrapped_cursor_warning_suggests_login_when_not_authenticated() {
        ...
        assert!(warning.contains("tokmesh cursor login"));
